  ...
  while true do
    ...
    for k=1,#sents do
      ...
      if opt.dump_images == 1 then
        ...
        local cmd = 'cp "' .. path.join(opt.image_root, data.infos[k].file_path) .. '" /data/output/vis/imgs/img' .. #predictions .. '.jpg' -- bit gross
        print(cmd)
...
if opt.dump_json == 1 then
  -- dump the json
  -- TODO put this output folder into a command line option
  utils.write_json('/data/output/vis/vis.json', split_predictions)
end
